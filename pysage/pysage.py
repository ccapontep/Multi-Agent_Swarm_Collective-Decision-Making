...
class PysageGUI(object):
    ...
    def initialize(self):
        ...
        a_width = self.pixels_per_meter*self.arena.dimensions.x
        a_height = self.pixels_per_meter*self.arena.dimensions.y
        c_midX = self.pixels_per_meter*(self.arena.dimensions.x/2)
        c_midY = self.pixels_per_meter*(self.arena.dimensions.y/2)


        self.w = tk.Canvas(self.master, width=int(a_width), height=int(a_height), background="#EEE")
        self.w.pack()
        self.w.create_oval(self.pixels_per_meter*0.05,self.pixels_per_meter*0.05,self.pixels_per_meter*(0.05+2*self.arena.size_radius),self.pixels_per_meter*(0.05+2*self.arena.size_radius), fill="white")
#        print ('canvas size', a_width, a_height)
